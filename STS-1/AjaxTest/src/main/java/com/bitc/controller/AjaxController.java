...
//	@ResponseBody : 해당 어노테이션을 사용 시 View를 반환하지 않고 데이터 자체를 반환
	@ResponseBody
//	value : RequestMapping 어노테이션에 여러개의 옵션을 사용할 경우 구분하기 위한 속성명, 클라이언트에서 요청하기 위한 URL을 입력하는 속성, RequestMapping에 단 하나의 속성만 사용 시 value가 기본이며, 삭제 가능함
...
//		Javascript의 object 타입과 비슷함, json데이터로 변환이 쉬움
	...
	@ResponseBody
	@RequestMapping(value="/ajax/ajaxTest.do", method=RequestMethod.POST)
	public Object ajaxTest() throws Exception {
		return "ajax Test text!!";
	}
	
	
	@RequestMapping(value="/ajax/calculator.do", method=RequestMethod.GET)
	public String ajaxCalculatorInput() throws Exception {
		return "/ajax/calculator";
	}
	
	@ResponseBody
	@RequestMapping(value="/ajax/calculator.do", method=RequestMethod.POST)
	public Object ajaxCalculatorResult(CalDto cal) throws Exception {
		...
		calResult.put("result", cal.getResult());
